...
public class Perlin {
	...
	static Point2D[] grad_tab = new Point.Float[TABLE_SIZE];
	...
	void init() {
		...
		for (i = 0; i < TABLE_SIZE; i++) {
			grad_tab[i] = new Point.Float();
			do {
				grad_tab[i].setLocation((r.nextDouble() * 2) - 1, (r.nextDouble() * 2) - 1);
				m = grad_tab[i].getX() * grad_tab[i].getX() + grad_tab[i].getY() * grad_tab[i].getY();
			} while (m == 0.0 || m > 1.0);

			m = 1.0 / Math.sqrt(m);
			grad_tab[i].setLocation(grad_tab[i].getX() * m, grad_tab[i].getY() * m);
		}
			...
			for (j = 0; j < 2; j++) {
				if (tilable)
					n = perm_tab[betterMod((betterMod((a + i), (clip * s))
							+ perm_tab[betterMod(betterMod((b + j), (clip * s)), TABLE_SIZE)]), TABLE_SIZE)];
				else
					n = perm_tab[betterMod(a + i + perm_tab[betterMod(b + j, TABLE_SIZE)], TABLE_SIZE)];
				v.setLocation(x - a - i, y - b - j);
				sum += WEIGHT(v.getX()) * WEIGHT(v.getY())
	...
	public static int[] generate(int w, int h) {
		...
		Perlin p = new Perlin(20);
		for (int x = 0; x < result.length; x++) {
			
			result[x] = (int) (p.noise(x % w, x % w));
		}
